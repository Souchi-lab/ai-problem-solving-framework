...
function Reset-RunStateToImproveNeeded {
    param(
        [string]$RunDir,
        [string]$Run
    )
...
TransitionService().transition(
    run_dir,
    to_phase="IMPROVE_NEEDED",
    actor="rerun",
    reason="apsf-rerun-improve: reset to IMPROVE_NEEDED",
)
ManifestRepository(run_dir).remove_entries(["improve.md", "result.md", "transcript.md"])
print("IMPROVE_NEEDED")
"@ | python -

    if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($result)) {
        throw "Failed to reset run_state.json to IMPROVE_NEEDED."
    }
}
...
Reset-RunStateToImproveNeeded -RunDir $runDir -Run $Run
