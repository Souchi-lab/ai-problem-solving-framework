...
function Reset-RunStateToPlanNeeded {
    param(
        [string]$RunDir,
        [string]$Run
    )
...
TransitionService().transition(
    run_dir,
    to_phase="PLAN_NEEDED",
    actor="rerun",
    reason="apsf-rerun-plan: reset to PLAN_NEEDED",
)
ManifestRepository(run_dir).remove_entries(["plan.md", "handoff.md", "build.md", "review.md", "improve.md", "result.md"])
print("PLAN_NEEDED")
"@ | python -

    if ($LASTEXITCODE -ne 0 -or [string]::IsNullOrWhiteSpace($result)) {
        throw "Failed to reset run_state.json to PLAN_NEEDED."
    }
}
...
Reset-RunStateToPlanNeeded -RunDir $runDir -Run $Run
